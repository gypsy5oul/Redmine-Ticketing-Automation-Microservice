...
async def get_workload_alerts(
    ...
    """Get workload alerts for overloaded members - Requires: Any authenticated user"""
    log_request(logger, request, user.id, "GET /api/v1/workload/alerts")
    try:
        manager = WorkloadManager(db)
        all_workload = manager.get_team_workload()

        # Filter for high load and at capacity members
        alerts = []
        for member_data in all_workload:
            if member_data["status"] in ["high_load", "at_capacity"]:
                alert_level = "critical" if member_data["status"] == "at_capacity" else "warning"
                alerts.append({
                    "alert_level": alert_level,
                    "member_id": member_data["user_id"],
                    "member_name": member_data["name"],
                    "team_level": member_data["team_level"],
                    "current_tickets": member_data["current_tickets"],
                    "max_tickets": member_data["max_tickets"],
                    "utilization": member_data["utilization"],
                    "status": member_data["status"],
                    "message": f"{member_data['name']} is {member_data['status'].replace('_', ' ')} ({member_data['utilization']}% utilization)"
                })
